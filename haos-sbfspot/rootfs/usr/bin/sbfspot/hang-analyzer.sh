...
# over two windows (24 h, 7 d) and writes them to /data/hangs.json (separate
# file — previously shared status.json with run-sbfspot.sh which created a
# write race when analyzer cron + poller completed within ~1s of each other).
# publish-heartbeat.sh reads BOTH files and publishes hangs_24h / hangs_7d
